...
async function saveHistory(action: string, detail: string, changedBy?: string) {
...
export async function POST(request: NextRequest) {
  ...
  const { action, month, year = 2026, changedBy } = body;
  const who = changedBy as string | undefined;
  ...
  try {
    ...
    if (action === "generate") {
      ...
      const to = month
        ...
        : `${year}-12-31`;
      await saveHistory("generate", `Generate from ${from}`, who);
    ...
    if (action === "unassign") {
      ...
      if (entry) {
        await saveHistory("unassign", `Unassign ${date}`, who);
    ...
    if (action === "notalk") {
      ...
      const [entry] = await db.select().from(rosterAssignments).where(eq(rosterAssignments.date, date));
      await saveHistory("no-talk", `No-talk ${date}`, who);
    ...
    if (action === "team_reading") {
      ...
      const [entry] = await db.select().from(rosterAssignments).where(eq(rosterAssignments.date, date));
      await saveHistory("team-reading", `Team reading ${date}`, who);
      const bumpedId = entry?.memberId ?? null;
      const bumpedTopic = entry?.topic ?? null;
    ...
    if (action === "restore") {
      ...
      if (entry && entry.status !== "scheduled") {
        await saveHistory("restore", `Restore ${date}`, who);
        await db.delete(rosterAssignments).where(eq(rosterAssignments.date, date));
    ...
    if (action === "swap") {
      ...
      if (entry?.memberId && swapWithMemberId) {
        await saveHistory("swap", `Swap ${date}`, who);
    ...
    if (action === "reorder_regenerate") {
      ...
      await saveHistory("reorder", `Reorder from ${from}`, who);
    ...
    if (action === "start_from_member") {
      ...
      if (date && memberId) {
        await saveHistory("start-from-member", `Start from ${memberId} on ${date}`, who);
    ...
    if (action === "pause") {
      ...
      await saveHistory("pause", `Pause ${startDate} to ${endDate}`, who);
    ...
    if (action === "reset_month") {
      ...
      await saveHistory("reset-month", `Reset ${y}-${m}`, who);
      await db.delete(rosterAssignments).where(
        and(
    ...
    if (action === "reset_year") {
      const y = Number(body.targetYear || year);
      await saveHistory("reset-year", `Reset ${y}`, who);
