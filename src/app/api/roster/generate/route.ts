...
export async function POST(request: NextRequest) {
  ...
  // Get active members by sort order (not HOD)
  ...
  if (presenters.length === 0) {
    return NextResponse.json(
      { error: "No active presenters found. Add team members first." },
      { status: 400 }
    );
  }
  ...
  // Get existing no-talk days so we don't overwrite them
  ...
  // Also get completed entries to not overwrite
  ...
  // Remove only "scheduled" assignments in the range
  await db
  ...
  // Build assignments skipping no-talk and completed days
  const values: Array<{
    date: string;
    memberId: number;
    status: string;
  ...
  for (const date of weekdays) {
    ...
    values.push({
      date,
      memberId: presenters[presenterIndex % presenters.length].id,
      status: "scheduled",
    });
    presenterIndex++;
  }
  ...
  return NextResponse.json({
    ...
    noTalkDaysSkipped: noTalkSet.size,
  });
}
