...
export async function GET() {
  await ensureTables();
  const allTopics = await db.select().from(topics).orderBy(topics.category, topics.title);
...
export async function POST(request: NextRequest) {
  ...
  const [newTopic] = await db
    .insert(topics)
    .values({ category, title, description: description || null, addedBy: addedBy || null })
    .returning();
