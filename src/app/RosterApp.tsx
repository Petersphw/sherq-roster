          ...
          <div className="anim-fade-up">
            ...
            {tab === "history" && (
              <div className="space-y-4">
                ...
                {!isAdmin ? (
                  <div className="glass-card rounded-2xl p-12 text-center"><p className="text-theme-muted">Only admins can view history.</p></div>
                ) : (
                  <div className="glass-card rounded-2xl overflow-hidden">
                      ...
                      {historyItems.length === 0 ? (
                        ...
                        <div key={h.id} className="px-4 py-3 flex items-center justify-between gap-3 hover:bg-theme-hover transition">
                          <div>
                            ...
                            <p className="text-[11px] text-theme-muted mt-1">{new Date(h.createdAt).toLocaleString("en-ZA")}</p>
                          </div>
                          <button onClick={() => undoToHistory(h.id)} className="px-3 py-2 text-xs font-semibold rounded-xl bg-[#1A4687] text-white hover:bg-[#225CB2] transition btn-press">Restore Here</button>
                        </div>
                      ))}
                    </div>
                  </div>
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
function EditModal({
  entry, members, topics, onSave, onSwap, onStartFromMember, onTeamReading, onRestore, onClose,
}: {
  entry: RosterEntry;
  members: Member[];
  topics: Topic[];
          ...
          <div>
            ...
            {mid && (
              <div className="mt-2 flex flex-wrap gap-2">
                <button
                  onClick={() => onStartFromMember(entry.date, mid)}
                  className="text-xs px-3 py-2 bg-[#6EC1E4]/10 text-[#1A4687] dark:text-[#6EC1E4] border border-[#6EC1E4]/30 rounded-xl font-medium hover:bg-[#6EC1E4]/20 transition btn-press"
                >
