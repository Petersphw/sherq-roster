...
export default function RosterApp({
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    // Auto-regenerate the roster with new order
    setBusy(true);
    await fetch("/api/roster/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ customStart: rosterStartDate || undefined }),
    });
    await fetchRoster(mo);
    await fetchStats();
    setBusy(false);
    toast("Roster reordered and regenerated ✓");
  };
  ...
  const setStartDate = async (date: string) => {
    setBusy(true);
    await fetch("/api/roster/settings", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    // Mark all weekdays before start date as "not-included" (NOT no-talk)
    ...
    // Regenerate from the new start date
    await fetch("/api/roster/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ customStart: date }),
    });

    await fetchRoster(mo);
    await fetchStats();
    setBusy(false);
    toast(`Roster starts from ${new Date(date + "T00:00:00").toLocaleDateString("en-ZA", { day: "numeric", month: "long", year: "numeric" })}`);
  };
  ...
  const pauseRange = async (startDate: string, endDate: string, reason: string) => {
    ...
    // Regenerate from the roster start date to fill gaps after pause
    await fetch("/api/roster/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ customStart: rosterStartDate || undefined }),
    ...
    setBusy(false);
    setPauseModal(false);
    toast("Roster paused — days marked and schedule shifted ✓");
  };

  const resetMonth = async (month: number) => {
    if (!confirm(`⚠️ Are you sure you want to reset ${MONTHS[month - 1]}?\n\nThis will DELETE all roster assignments for that month (scheduled, completed, no-talk days — everything).\n\nThis cannot be undone.`)) return;
    if (!confirm(`FINAL CONFIRMATION:\n\nReset ALL of ${MONTHS[month - 1]} 2026?\n\nClick OK to proceed.`)) return;
    setBusy(true);
    await fetch("/api/roster/reset", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    await fetchStats();
    setBusy(false);
    toast(`${MONTHS[month - 1]} has been reset`);
  };
...
function CalendarView({
  ...
  busy, teamsCalUrl, resetMonth, rosterStartDate, setStartDate, setPauseModal,
}: {
  roster: RosterEntry[];
  mems: Member[];
  mo: number;
