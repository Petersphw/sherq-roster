...
export default function RosterApp({
  ...
  useEffect(() => { if (seeded) { fetchRoster(mo, yr); fetchStats(); fetchWeather(); fetchSettings(); } }, [mo, yr, seeded, fetchRoster, fetchStats]);
  ...
  const rosterAction = async (actionBody: Record<string, unknown>, toastMsg?: string) => {
    ...
    // Stats refresh in background (non-blocking)
    fetchStats();
  };
  ...
  const unassignDate = async (date: string) => {
    ...
    await rosterAction({ action: "unassign", date }, "Unassigned — roster shifted ✓");
  };
  ...
  const markNoTalk = async (
    ...
    setNoTalkModal(null);
    await rosterAction({ action: "notalk", date, reason, altPresenter, altTopic, notes }, "No-talk day set ✓");
  };
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    await rosterAction({ action: "generate", customStart: rosterStartDate || undefined }, "Roster reordered ✓");
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="glass-card rounded-2xl p-5 shadow-theme anim-fade-up delay-1">
            ...
            <div className="flex flex-wrap gap-3">
              <button
                onClick={() => generate(mo)}
                disabled={busy}
                className="px-4 py-2.5 gradient-premium text-white btn-press rounded-xl text-sm font-medium transition-all shadow-lg hover:shadow-xl disabled:opacity-50 flex items-center gap-2"
              >
          ...
          {/* Next Talk */}
          {nextTalk && (
            <div className="glass-card rounded-2xl p-5 shadow-theme anim-fade-up delay-2">
              <h3 className="font-bold text-theme-primary mb-4 flex items-center gap-2">
        ...
        <div className="space-y-4">
          ...
          {stats && stats.perMember.length > 0 && (
            <div className="glass-card rounded-2xl p-5 shadow-theme anim-fade-up delay-4">
              <h3 className="font-bold text-theme-primary mb-4 flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-3 p-3">
                        ...
                        <div className="flex gap-1 items-center">
                          {m.email === "PeterSM@sas.co.za" && (
                            <span className="text-[9px] px-1.5 py-0.5 rounded-full font-bold bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-400 border border-violet-200 dark:border-violet-800">
                              ⚡ Developer
                            </span>
                          )}
                          {m.isAdmin && m.email !== "PeterSM@sas.co.za" && (
                            <span className="text-[9px] px-1.5 py-0.5 rounded-full font-bold bg-[#1A4687]/10 dark:bg-[#6EC1E4]/10 text-[#1A4687] dark:text-[#6EC1E4] border border-[#1A4687]/20 dark:border-[#6EC1E4]/20">
                              🛡️ Admin
                            </span>
                          )}
                          {m.pin && (
                            <span className="text-[9px] px-1.5 py-0.5 rounded-full font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                              🔒 PIN
                            </span>
                          )}
                          {m.pinResetRequested && (
                          ...
                          {m.role === "HOD" && !m.isAdmin && m.email !== "PeterSM@sas.co.za" && (
                            <span className="text-[9px] px-1.5 py-0.5 rounded-full font-bold bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800">
                              👔 HOD
                            </span>
                          )}
      ...
      <div className="glass-card rounded-3xl shadow-2xl w-full max-w-lg border border-theme anim-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          <div>
            ...
            <select value={mid ?? ""} onChange={(e) => setMid(e.target.value ? +e.target.value : null)} className="w-full px-3 py-2.5 bg-theme-secondary border border-theme rounded-xl text-sm text-theme-primary input-premium outline-none">
          ...
          {entry.memberId && entry.status === "scheduled" && (
            <div>
              <button onClick={() => setShowSwap(!showSwap)} className="text-[10px] font-semibold text-theme-muted uppercase tracking-wider flex items-center gap-1 hover:text-theme-secondary transition mb-1.5">
                🔄 {showSwap ? "Hide swap" : "Swap presenter"} <span className="text-theme-muted/50 font-normal normal-case">(absent / late / technical issues)</span>
              </button>
              {showSwap && (
                <div className="p-3 bg-theme-secondary rounded-xl border border-theme anim-scale">
                  <p className="text-xs text-theme-muted mb-2">Swap <strong>{entry.memberName}</strong> with another member. They only trade these two dates — the rest of the roster stays as is.</p>
                  <div className="grid grid-cols-2 gap-2">
                    {members.filter((m) => m.id !== entry.memberId).map((m) => (
                      <button
                        key={m.id}
                        onClick={() => { onSwap(entry.date, m.id); }}
                        className="text-xs px-3 py-2 bg-theme-card border border-theme rounded-xl hover:border-[#6EC1E4] hover:bg-[#6EC1E4]/5 transition text-left font-medium text-theme-primary btn-press"
                      >
                        🔄 {m.name}
                      </button>
                    ))}
        ...
        <div className="px-5 py-4 border-t border-theme bg-theme-secondary rounded-b-2xl flex items-center gap-2">
          {entry.id > 0 && (
            <button
              onClick={() => { if (confirm("Remove this date and shift the rest of the roster?")) { onUnassign(entry.date); onClose(); } }}
              className="px-3 py-2.5 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 text-xs font-medium rounded-xl transition mr-auto"
            >
              🗑️ Unassign & Shift
            </button>
          )}
