...
type Tab = "dashboard" | "calendar" | "roster" | "members" | "topics" | "messages" | "history";
...
export default function RosterApp({
  ...
  const fetchHistory = async () => {
    const d = await safeFetch(`/api/history`);
    if (d) setHistoryItems(d);
  };
  ...
  const rosterAction = async (actionBody: Record<string, unknown>, toastMsg?: string) => {
    const payload = { ...actionBody, month: mo, year: yr, changedBy: currentUser?.name || "Unknown" };
    try {
  ...
  const startFromMember = async (date: string, memberId: number) => {
    setEditing(null);
    await rosterAction({ action: "start_from_member", date, memberId }, "Roster resequenced from selected member ✓");
  };
  ...
  const undoLast = async () => {
    if (!confirm("Undo the last roster action?")) return;
    await rosterAction({ action: "undo" }, "Undid last action ✓");
  };

  const undoToHistory = async (historyId: number) => {
    if (!confirm("Restore roster to this point in history?")) return;
    try {
      ...
      await Promise.all([fetchRoster(mo, yr), fetchDashboardRoster(), fetchStats(), fetchHistory()]);
      toast("History restored ✓");
    } catch {
      toast("History restore failed");
    }
  };
      ...
      <div className="min-h-screen bg-theme-secondary relative">
        ...
        <main className={`relative z-10 max-w-7xl mx-auto px-4 py-8 transition-all duration-300 ${!isLoggedIn && seeded ? "blur-sm opacity-60 pointer-events-none select-none" : ""}`}>
          <div className="anim-fade-up">
                      ...
                      {mems.filter((m) => m.active && m.id !== currentUser.id).map((m) => {
                        ...
                        return (
                          <button key={m.id} onClick={() => { setChatWith({ id: m.id, name: m.name }); fetchChat(m.id); }}
                            ...
                            <div className="flex-1 min-w-0"><p className="text-sm font-medium text-theme-primary truncate">{m.name}</p></div>
            ...
            {tab === "history" && (
              <div className="space-y-4">
                <h2 className="text-xl font-bold text-theme-primary flex items-center gap-2 anim-fade-down">↩️ History</h2>
                {!isAdmin ? (
                  <div className="glass-card rounded-2xl p-12 text-center"><p className="text-theme-muted">Only admins can view and restore history.</p></div>
                ) : (
                  <div className="glass-card rounded-2xl overflow-hidden">
                    <div className="px-4 py-3 border-b border-theme bg-gradient-to-r from-[#1A4687]/5 dark:from-[#6EC1E4]/5 to-transparent flex items-center justify-between">
                      <p className="text-sm font-semibold text-theme-primary">Roster History</p>
                      <button onClick={undoLast} className="text-xs px-3 py-1.5 bg-theme-secondary border border-theme rounded-lg text-theme-secondary hover:bg-theme-hover">Undo Last</button>
                    </div>
                    <div className="divide-y divide-theme max-h-[520px] overflow-y-auto">
                      {historyItems.length === 0 ? (
                        <div className="p-8 text-center text-theme-muted text-sm">No history yet</div>
                      ) : historyItems.map((h: {id:number;action:string;detail:string|null;changedBy:string|null;createdAt:string}) => (
                        <div key={h.id} className="px-4 py-3 flex items-center justify-between gap-3 hover:bg-theme-hover transition">
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              <p className="text-sm font-medium text-theme-primary">{h.action}</p>
                              {h.changedBy && <span className="text-[10px] px-2 py-0.5 rounded-full bg-theme-secondary text-theme-muted">by {h.changedBy}</span>}
                            </div>
                            <p className="text-xs text-theme-muted">{h.detail || "Roster change"}</p>
                            <p className="text-[11px] text-theme-muted mt-1">{new Date(h.createdAt).toLocaleString("en-ZA", { timeZone: "Africa/Johannesburg" })}</p>
                          </div>
                          <button onClick={() => undoToHistory(h.id)} className="px-3 py-2 text-xs font-semibold rounded-xl bg-[#1A4687] text-white hover:bg-[#225CB2] transition btn-press shrink-0">Restore Here</button>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        {editing && (
          <EditModal
            ...
            onSwap={swapMembers}
            onStartFromMember={startFromMember}
...
function CalendarView({
  ...
  busy, teamsCalUrl, resetMonth, resetYear, rosterStartDate, setStartDate, setPauseModal, isAdmin, undoLast,
}: {
  roster: RosterEntry[];
  mems: Member[];
  mo: number;
    ...
    <div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {isAdmin && (
            <button
              onClick={undoLast}
              disabled={busy}
              className="px-4 py-2.5 bg-theme-card border border-theme text-theme-secondary hover:bg-theme-hover rounded-xl text-xs font-medium transition-all disabled:opacity-50"
            >
              ↩️ Undo
            </button>
          )}
      ...
      {rosterStartDate && (
        <div className="mb-3 flex items-center gap-2 text-xs text-theme-secondary">
          ...
          {isAdmin && (
            <button onClick={() => { setStartDate(`${yr}-01-13`); }} className="text-theme-muted hover:text-rose-500 text-[10px] underline">Clear</button>
          )}
        </div>
      )}
      ...
      {settingStart && (
        <div className="mb-3 p-3 glass-card rounded-xl anim-scale flex items-center gap-3 border-2 border-[#6EC1E4]/50 anim-glow">
          ...
          <p className="text-sm text-theme-primary font-medium flex-1">Click any weekday on the calendar below to set it as the <strong>roster start date</strong>. Everything before it will be marked as <strong>not included</strong>.</p>
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
function EditModal({
  entry, members, topics, onSave, onSwap, onStartFromMember, onTeamReading, onRestore, isAdmin, currentUserId, onClose,
}: {
  entry: RosterEntry;
  members: Member[];
  topics: Topic[];
  ...
  onStartFromMember: (date: string, memberId: number) => void;
  ...
  // Non-admin can only set topic on their own scheduled day
  const isOwnDay = entry.memberId === currentUserId && entry.status === "scheduled";
  const canEdit = isAdmin || isOwnDay;
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-md z-50 flex items-center justify-center p-4 anim-modal-bg" onClick={onClose}>
      <div className="glass-card rounded-3xl shadow-2xl w-full max-w-lg border border-theme anim-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          {/* Non-admin: can only set topic on their own day */}
          {!canEdit && (
            <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-xl p-3 text-sm text-amber-700 dark:text-amber-300">
              {isOwnDay ? "You can set your topic below." : "Only admins can edit assignments. You can view this entry."}
            </div>
          )}

          {/* Presenter — admin only */}
          {isAdmin && (
            <div>
              ...
              {mid && (
                <div className="mt-2">
                  <button onClick={() => onStartFromMember(entry.date, mid)} className="text-xs px-3 py-2 bg-[#6EC1E4]/10 text-[#1A4687] dark:text-[#6EC1E4] border border-[#6EC1E4]/30 rounded-xl font-medium hover:bg-[#6EC1E4]/20 transition btn-press">
                    ▶ Start from this member here
                  </button>
                </div>
              )}
            </div>
          ...
          {/* Topic — admin always, member only on own day */}
          {canEdit && (
            <div>
          ...
          {/* Swap — admin only */}
          {isAdmin && entry.memberId && entry.status === "scheduled" && (
            <div>
        ...
        <div className="px-5 py-4 border-t border-theme bg-theme-secondary rounded-b-2xl flex items-center gap-2">
          {/* Restore — admin only */}
          {isAdmin && (entry.status === "no-talk" || (entry.status === "completed" && entry.noTalkReason)) && (
            <button
          ...
          <button onClick={onClose} className="px-4 py-2.5 text-theme-secondary hover:text-theme-primary text-sm font-medium rounded-xl hover:bg-theme-hover transition">{canEdit ? "Cancel" : "Close"}</button>
          {canEdit && (
            <button onClick={() => onSave(entry.date, isAdmin ? mid : entry.memberId, top || null, isAdmin ? st : entry.status, isAdmin ? notes || null : entry.notes)} className="px-5 py-2.5 bg-gradient-to-r from-[#1A4687] to-[#225CB2] text-white rounded-xl text-sm font-semibold transition-all shadow-lg hover:shadow-xl">
              ⚓ {isOwnDay && !isAdmin ? "Set Topic" : "Save"}
            </button>
          )}
        </div>
      </div>
